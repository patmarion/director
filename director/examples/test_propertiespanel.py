...
def main():
    ...
    props.addProperty('double_precise', 1.0, attributes=PropertyAttributes(decimals=5, minimum=0, maximum=100, singleStep=0.001))
    props.addProperty('double_rounded', 1.0, attributes=PropertyAttributes(decimals=2, minimum=0, maximum=100, singleStep=0.1))
    props.addProperty('double_large_step', 10.0, attributes=PropertyAttributes(decimals=1, minimum=-1000, maximum=1000, singleStep=10.0))
    ...
    props.addProperty('enum_choice', 0, attributes=PropertyAttributes(enumNames=['Option A', 'Option B', 'Option C', 'Option D']))
    props.addProperty('quality', 1, attributes=PropertyAttributes(enumNames=['Low', 'Medium', 'High', 'Ultra']))
    
    # Arrays with attributes
    props.addProperty('double list', [1.0, 2.0, 3.0], attributes=PropertyAttributes(decimals=2, minimum=0, maximum=100, singleStep=0.5))
    ...
    props.addProperty('nest2/level1/float_precise', 1.23456, attributes=PropertyAttributes(decimals=4, minimum=0, maximum=10, singleStep=0.0001))
