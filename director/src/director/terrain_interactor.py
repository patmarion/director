...
class TerrainInteractorStyle(vtk.vtkInteractorStyle):
    ...
    def _rotate_camera(self, camera, dx, dy, renderer=None):
        ...
        new_position = [
            focal_point[i] + new_view_vector[i] for i in range(3)
        ]
        ...
        view_up = [
            reference[i] - dot_product * view_dir_normalized[i]
            for i in range(3)
        ]
        ...
        if abs(view_dir_normalized[2]) > 0.9:
            print(f"View dir: [{view_dir_normalized[0]:.3f}, {view_dir_normalized[1]:.3f}, {view_dir_normalized[2]:.6f}]")
    ...
    def _pan_camera(self, camera, dx, dy, renderer=None):
        ...
        vtk.vtkInteractorObserver.ComputeWorldToDisplay(
            renderer, fp[0], fp[1], fp[2], focal_point_display)
    ...
    def _ray_plane_intersection(self, ray_start, ray_end, plane_point, plane_normal):
        ...
        intersection = [
            ray_start[i] + t * ray_dir_normalized[i] * ray_length
            for i in range(3)
        ]
