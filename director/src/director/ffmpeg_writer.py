...
class FFMpegWriter:
    ...
    def __init__(self, filename: str, width: int, height: int, framerate: float = 30.0,
                 vcodec: str = 'libx264', preset: str = 'slow', crf: int = 18,
    ...
    def write_frame(self, frame: np.ndarray):
        ...
        if frame.shape != (self.height, self.width, 3):
            raise ValueError(
                f"Frame shape {frame.shape} does not match expected ({self.height}, {self.width}, 3)"
            )
    ...
    def close(self):
        ...
        try:
            ...
            if return_code != 0:
                ...
                stderr_output = self.process.stderr.read().decode('utf-8', errors='ignore')
                raise RuntimeError(
                    f"FFMpeg process exited with code {return_code}. "
