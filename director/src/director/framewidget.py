...
class FrameWidget(ViewEventFilter):
    ...
    def _buildActors(self):
        ...
        for ringId in range(3):
            polyData = createRingGeometry(self.scale, handleRadius, ringId, useTubeFilter=not self.useDiskRings)
    ...
    def _translateAlongAxis(self, axisId, deltaX, deltaY):
        ...
        vtk.vtkLine.Intersection(
            currentRay1.tolist(), currentRay2.tolist(), startWorldPoint.tolist(), lineEnd.tolist(), uu, vv
        )
        ...
        prevRay1, prevRay2 = self._getWorldRay(self.lastEventPosition[0], self.lastEventPosition[1])

        # Compute intersection for previous position
        uu2 = mutable(0.0)
        vv2 = mutable(0.0)
        vtk.vtkLine.Intersection(
            prevRay1.tolist(), prevRay2.tolist(), startWorldPoint.tolist(), lineEnd.tolist(), uu2, vv2
        )
    ...
    def _rotateAboutAxis(self, axisId, deltaX, deltaY):
        ...
        vtk.vtkInteractorObserver.ComputeWorldToDisplay(
            self.view.renderer(), centerOfRotation[0], centerOfRotation[1], centerOfRotation[2], displayCenter
        )
        ...
        vec1 = np.array([currentX - displayCenter[0], currentY - displayCenter[1], 0.0])
        vec2 = np.array(
            [self.lastEventPosition[0] - displayCenter[0], self.lastEventPosition[1] - displayCenter[1], 0.0]
        )
    ...
    def _translateInPlane(self, ringId, deltaX, deltaY):
        ...
        vtk.vtkPlane.IntersectWithLine(
            currentRay1.tolist(),
            currentRay2.tolist(),
            planeNormalWorld.tolist(),
            startWorldPoint.tolist(),
            t,
            planePoint,
        )
