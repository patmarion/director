...
from director import applogic, filterUtils, imageview, transformUtils, viewbehaviors
...
class DepthScanner:
    ...
    def update(self):
        ...
        depthImage, polyData = computeDepthImageAndPointCloud(
            self.getDepthBufferImage(), self.getColorBufferImage(), self.view.camera()
        )
        ...
        if not self.pointCloudObj:
            self.pointCloudObj = vis.showPolyData(
                polyData, "point cloud", colorByName="rgb", view=self.pointCloudView, parent=self.parentFolder
            )
        else:
...
def initCameraFrustumVisualizer(depthScanner):
    cameraObj = vis.showPolyData(
        vtk.vtkPolyData(), "camera", parent=depthScanner.parentFolder, view=depthScanner.pointCloudView
    )
    ...
    def updateCameraMesh():
        ...
        cameraMesh = filterUtils.transformPolyData(
            cameraMesh, getCameraTransform(depthScanner.view.camera()).GetLinearInverse()
        )
    ...
    def onCameraModified():
        ...
        depthScanner.pointCloudObj.actor.SetUserTransform(
            transformUtils.concatenateTransforms([pointCloudToCamera, cameraToWorld])
        )
...
def main(globalsDict=None):
    ...
    def addTestData():
        ...
        d.addArrow((0, 0, 1), (0, 0.5, 1), color=[0, 1, 0])
