class ViewBackgroundLightHandler:
    def __init__(self, viewOptions, grid, action):
        ...
        self.action.triggered.connect(self._onChecked)

        self.properties = {
            viewOptions: {
                "Gradient background": True,
                "Background color": [0.5, 0.52, 0.55],
                "Background color 2": [1.0, 1.0, 1.0],
            },
            grid: {
                "Surface Mode": "Surface with edges",
                "Alpha": 0.15,
                "Color": [0.0, 0.0, 0.0],
            },
        }
    ...
    def isEnabled(self):
        return bool(self.action.isChecked())
