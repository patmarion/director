...
class VectorInterpolator():
    ...
    def __init__(self, startValue, targetValue, callback):
        ...
        self.startValue = np.array(startValue)
...
class MyCameraInteractor(vieweventfilter.ViewEventFilter):
    ...
    def onMouseMove(self, event):
        ...
        vis.updatePolyData(d.getPolyData(), 'hit point', color=[1.0, 0.0, 0.0])
    ...
    def flyTo2(self, targetPosition):
        ...
        targetPosition = np.array([*targetPosition, length*(1-zoom)])

        if self.interp is None:
            self.interp = VectorInterpolator(focalPoint, targetPosition, f)
        else:
...
c = CameraInterpolator(view)

i = MyCameraInteractor(view)
